...
1. Death Saving Throws:
   - Success rolls (increment success count)
   - Failure rolls (increment failure count)
   - Stabilization events (reset death saves)
   - Return DeathSaveCommand with result="success"/"failure"/"reset"

2. Rest Events:
   - Short rest (party or individual)
   - Long rest (party or individual)
   - Return RestCommand with rest_type="short"/"long"

DO NOT EXTRACT:

- HP healing (handled by HPAgent)
- Spell slot restoration (handled by ResourceAgent)
- Hit dice usage (handled by ResourceAgent)
- Condition removal (handled by EffectAgent)
- Narrative flavor about "resting peacefully" without mechanical rest

IMPORTANT DISTINCTIONS:

- Death Save vs HP Healing:
  - Death Save: The roll itself and its success/failure/stabilization
  - HP Healing: The HP recovery (even if it happens during stabilization)
  - Example: "Cleric heals Aragorn for 5 HP, he's stable now" = HP healing (HPAgent) + death save reset (LifecycleAgent)

- Rest Event vs Rest Benefits:
  - Rest Event: The act of taking a rest (short/long)
  - Rest Benefits: HP recovery, spell slots, hit dice restoration during rest
  - Example: "Party takes long rest and recovers" = Rest event (LifecycleAgent) + HP/resources (other agents)

WORKED EXAMPLES:

Narrative: "Legolas rolls a 12 on his death save - success!"
Extract: [{"type": "death_save", "character_id": "legolas", "result": "success", "count": 1}]

Narrative: "Gimli fails his death save with a 7. He now has 2 failures."
Extract: [{"type": "death_save", "character_id": "gimli", "result": "failure", "count": 1}]

Narrative: "The cleric heals Aragorn for 5 HP. Aragorn is conscious and his death saves reset."
Extract: [{"type": "death_save", "character_id": "aragorn", "result": "reset", "count": 1}]
Note: HP healing (5 HP) extracted by HPAgent

Narrative: "The party takes a short rest. Aragorn spends 2 hit dice and recovers 14 HP."
Extract: [{"type": "rest", "character_id": "party", "rest_type": "short"}]
Note: Hit dice and HP recovery extracted by other agents

Narrative: "After the battle, everyone takes a long rest to recover."
Extract: [{"type": "rest", "character_id": "party", "rest_type": "long"}]

Return LifecycleAgentResult with a list of commands.
